...
{
    /// <summary>
    /// Gestisce i filtri di visilità degli accordi per il capo intermedio.
    /// L'utente deve poter visualizzare unicamente gli accordi di cui è responsabile.
    /// </summary>
    public class AccordoKEY_CLAIM_CAPO_INTERMEDIOVisibilityHandler : IVisibilityHandler<Accordo, Guid>
    {
        /// <summary>
        ...
        /// <returns>Queryable con la clausola di filtro per il capo intermedio.</returns>
        ...
        {
            var listaStrutture = strutturaService.FindStrutturaAsync(s => s.CapoIntermedio.Id.Equals(uid)).GetAwaiter().GetResult();
            ...
            predicate = predicate.Or(a => a.CapoIntermedio.Id.Equals(uid));
